...
<div class="row post-sections" id="posts-section" style="min-height:500px;">
	<div class="large-12 medium-12 small-12 columns">
		...
		{{-- <ul>
			...
			<li>
				<a class="post-btn" href="{{ URL::route('workdataitem', array($post->id)) }}">{{$post->name}}</a>
			</li>
			@endforeach
		</ul> --}}
		...
		<span style="display: block;"><strong>Address</strong><span>: </span>{!! $post->address !!}</span> --}}
                    <a href="{{ URL::route('workdataitem', array($post->id)) }}" style="display: block; padding: 5px; border-radius: 3px; background: #c89b62; font-size: 14px; font-weight: bold; text-align: center; color: white; text-transform: uppercase; margin-top: 10px;">read more</a>
                </li>
			@endforeach
            </ul>
